"""Tahminci profili — kişi/kurum bazında tahmin geçmişi ve performans."""
import pandas as pd
...
st.markdown(
    """
    <div class="hero">
      <h1>👤 Tahminci Profili</h1>
      <p>Bir kişi veya kurumun tüm tahmin geçmişi, ortalama hatası ve son
         tahmin tarihleri.</p>
    </div>
    """,
...
if profiles.empty:
    ...
    c1.metric("Toplam tahmin", int(p.get("forecast_count") or 0))
    c2.metric("Olay sayısı", int(p.get("event_count") or 0))
    avg_err = p.get("avg_abs_error")
    c3.metric("Ort. mutlak hata", f"{avg_err:.2f}" if pd.notna(avg_err) else "-")
    ...
    if not pf.empty:
        ...
        excel_download_button(
            pf, f"{selected}_tahminleri.xlsx", "Bu tahmincinin verilerini Excel indir",
        )
