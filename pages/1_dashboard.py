"""Ana dashboard — KPI'lar, trend, ısı haritası, madalya kürsüsü, boxplot.

NOT: Aşama 3'te bu sayfa hafifletilecek, içerik bir kısmı Analytics hub'ına taşınacak.
"""
import pandas as pd
...
from utils.export import add_display_labels, apply_common_filters, excel_download_button
from utils.ui import inject_theme, plot_layout, styled_line
...
st.markdown(
    ...
    <div class="hero">
      <h1>📈 Dashboard</h1>
      <p>Filtrelenebilir tahminler, gerçekleşmeler, trendler, ısı haritası ve
         madalya kürsüsü.</p>
    </div>
    """,
...
c1.metric("Tahmin sayısı", len(dff))
...
trend_mode = st.radio(
    "Trend görünümü", ["Katılımcı bazlı", "Gösterge ortalaması"], horizontal=True,
)
if trend_mode == "Katılımcı bazlı":
    fig = styled_line(
        dff.sort_values("target_period"),
        x="target_period", y="forecast_value",
        color="participant_name", title="Tahmin trendi",
    )
else:
    avg = dff.groupby(["target_period", "target_type_label"], as_index=False).agg(
        forecast_value=("forecast_value", "mean"),
        actual_value=("actual_value", "mean"),
    )
    fig = styled_line(
        avg.sort_values("target_period"),
        x="target_period", y="forecast_value",
        color="target_type_label",
        title="Ortalama beklenti trendi", fill=True,
    )
...
if not actual.empty:
    fig.add_trace(
        go.Scatter(
            x=actual["target_period"], y=actual["actual_value"],
            mode="lines+markers", name="Gerçekleşme",
            line=dict(width=4, color="#111827", dash="dot", shape="spline"),
...
if not box_df.empty:
    ...
    fig_box = px.box(
        box_df,
        x="group_label", y="forecast_value", points="all",
...
heat = dff.dropna(subset=["abs_error"]).pivot_table(
    index="participant_name", columns="target_period",
    values="abs_error", aggfunc="mean",
)
if not heat.empty:
    fig2 = px.imshow(
        heat, aspect="auto",
        labels=dict(color="Mutlak hata"),
